	...
	mGrid(window, Vector2i(5, 5)), mBusyAnim(window)
{
	...
	mGrid.setEntry(mMD_Grid, Vector2i(3, 1), false, false);
...
void ScraperSearchComponent::onSizeChanged()
{
	...
	mGrid.setColWidthPerc(1, 0.02f);
	mGrid.setColWidthPerc(2, 0.24f);
	mGrid.setColWidthPerc(3, 0.24f);
	...
	mResultThumbnail->setMaxSize(mGrid.getColWidth(2) * boxartCellScale, mGrid.getRowHeight(1));
...
{
	mMD_Grid->setSize(mGrid.getColWidth(3), mGrid.getRowHeight(1));
	if (mMD_Grid->getSize().y() > mMD_Pairs.size())
	{
		...
		mMD_Rating->setSize(mMD_Grid->getColWidth(2), font->getHeight() * 0.65f);
...
void ScraperSearchComponent::updateViewStyle()
{
	...
	// fake row where name would be
	mGrid.setEntry(std::make_shared<GuiComponent>(mWindow), Vector2i(1, 4), false, true, Vector2i(4, 1), GridFlags::BORDER_BOTTOM);
}

/// This class is used to call Scraper->search() in a thread. 
/// This avoids freezes while building requests, as they can have some blocking code ( build rom md5, ask server for a token... )
class ScraperAsyncRequestsBuilder : public ScraperSearchHandle
{
public:
	ScraperAsyncRequestsBuilder(Scraper* scraper, ScraperSearch* ss)
	{
		mScraper = scraper;		
		mSearch = ss;

		mFinished = false;
		mThread = new std::thread(&ScraperAsyncRequestsBuilder::loadSearchHandleAsync, this);
	}
	
	void loadSearchHandleAsync()
	{
		try
		{
			mSearchHandle = mScraper->search(mSearch->params);
		}
	...
	virtual void update()
	{
		...
		if (mSearchHandle == nullptr)
			setError(mError);
		else
			setStatus(ASYNC_DONE);
	}

	ScraperSearchHandle* ExtractSearchHandle()
	{
		if (mSearchHandle)
			return mSearchHandle.release();
...
{
	if (mSearchStartingCallback)
		mSearchStartingCallback();
...
void ScraperSearchComponent::update(int deltaTime)
{
	...
	for (auto engine : mScrapEngines)
	{
		...
		if (status == ASYNC_DONE && results.size() == 0 && engine->params.nameOverride.empty())
		{
			ScraperAsyncRequestsBuilder* handle = dynamic_cast<ScraperAsyncRequestsBuilder*>(engine->searchHandle.get());
			if (handle != nullptr)
			{
				ScraperSearchHandle* newHandle = handle->ExtractSearchHandle();
				if (newHandle != nullptr)
					engine->searchHandle = std::unique_ptr<ScraperSearchHandle>(newHandle);
				else 
