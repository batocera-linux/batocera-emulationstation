...
//////////////////////////////////////////////////////////////////////////
	...
	static size_t			vertexBufferOffset = 0;
	...
	static const size_t INITIAL_STREAM_BUFFER_SIZE = 256 * 1024;
	static const size_t STREAM_MAP_THRESHOLD = 4 * 1024;
	...
	static const size_t MAX_BATCH_VERTICES = 6144;
...
//////////////////////////////////////////////////////////////////////////
	...
	static void useProgram(ShaderProgram* program)
	{
		...
		{
			currentProgram->select(vertexBuffer);
			currentProgram->setMatrix(mvpMatrix);
			mvpMatrixDirty = false;
		}
	}
...
//////////////////////////////////////////////////////////////////////////
	...
	static void setupVertexBuffer()
	{
		...
		vertexBufferCapacity = INITIAL_STREAM_BUFFER_SIZE;
		...
		while (glGetError() != GL_NO_ERROR)
			;
		glBufferData(GL_ARRAY_BUFFER, vertexBufferCapacity, nullptr, GL_STREAM_DRAW);
		if (glGetError() != GL_NO_ERROR)
		{
			LOG(LogError) << "Unable to allocate the OpenGL ES 3.0 streaming vertex buffer";
	...
	static StreamRange uploadVertices(const Vertex* vertices, const size_t vertexCount, const bool updateLogicalRange = false)
	{
		...
		const size_t dataSize = vertexCount * sizeof(GpuVertex);
		GLES30_CALL(glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer));

		if (dataSize > vertexBufferCapacity)
		{
			size_t newCapacity = vertexBufferCapacity == 0 ? INITIAL_STREAM_BUFFER_SIZE : vertexBufferCapacity;
			while (newCapacity < dataSize)
			{
				if (newCapacity > std::numeric_limits<size_t>::max() / 2)
				{
					newCapacity = dataSize;
					break;
				}
				newCapacity *= 2;
			}
			...
			while (glGetError() != GL_NO_ERROR)
				;
			glBufferData(GL_ARRAY_BUFFER, newCapacity, nullptr, GL_STREAM_DRAW);
			if (glGetError() != GL_NO_ERROR)
			{
				LOG(LogError) << "Unable to grow the OpenGL ES 3.0 streaming vertex buffer to " << newCapacity << " bytes";
				return range;
			}
		...
		else if (vertexBufferOffset + dataSize > vertexBufferCapacity)
		{
			while (glGetError() != GL_NO_ERROR)
				;
			glBufferData(GL_ARRAY_BUFFER, vertexBufferCapacity, nullptr, GL_STREAM_DRAW);
			if (glGetError() != GL_NO_ERROR)
			{
				LOG(LogError) << "Unable to orphan the OpenGL ES 3.0 streaming vertex buffer";
				return range;
			}
			vertexBufferOffset = 0;
			++streamBufferGeneration;
		...
		bool uploaded = false;
		if (dataSize >= STREAM_MAP_THRESHOLD)
		{
			GpuVertex* mapped = static_cast<GpuVertex*>(glMapBufferRange(GL_ARRAY_BUFFER, vertexBufferOffset, dataSize,
				GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_UNSYNCHRONIZED_BIT));
			if (mapped != nullptr)
			{
				for (size_t i = 0; i < vertexCount; ++i)
					mapped[i] = packVertex(vertices[i]);

				if (glUnmapBuffer(GL_ARRAY_BUFFER) == GL_TRUE)
					uploaded = true;
				else
					LOG(LogError) << "OpenGL ES 3.0 streaming vertex-buffer mapping became invalid";
			}
			else
			{
				// Mapping is an optimization. Clear its error and retain a reliable upload fallback.
				while (glGetError() != GL_NO_ERROR)
					;
			}
		}
		...
		if (!uploaded)
		{
			...
			GLES30_CALL(glBufferSubData(GL_ARRAY_BUFFER, vertexBufferOffset, dataSize, packed.data()));
		}

		range.firstVertex = static_cast<GLint>(vertexBufferOffset / sizeof(GpuVertex));
		range.vertexCount = static_cast<GLsizei>(vertexCount);
		range.generation = streamBufferGeneration;
		range.valid = true;

		vertexBufferOffset += dataSize;
		if (updateLogicalRange)
		{
...
//////////////////////////////////////////////////////////////////////////
	...
	static void drawStreamRange(const GLenum mode, const StreamRange& range, const Blend::Factor source, const Blend::Factor destination)
	{
		...
		GLES30_CALL(glDrawArrays(mode, range.firstVertex, range.vertexCount));
	}
	...
	static void flushQuadBatch()
	{
		...
		const StreamRange range = uploadVertices(quadBatch.vertices.data(), quadBatch.vertices.size(), false);
	...
	static void rememberLogicalVertices(const Vertex* vertices, const size_t vertexCount)
	{
		lastLogicalVertices.assign(vertices, vertices + vertexCount);
		lastStreamRange.valid = false;
	}
		...
		const QuadBatchShader shader)
	{
		...
		// A four-vertex strip is two triangles with alternating strip winding.
		// Expanding it avoids connector triangles when adjacent quads are merged.
		quadBatch.vertices.push_back(vertices[0]);
		quadBatch.vertices.push_back(vertices[1]);
		quadBatch.vertices.push_back(vertices[2]);
		quadBatch.vertices.push_back(vertices[2]);
		quadBatch.vertices.push_back(vertices[1]);
		quadBatch.vertices.push_back(vertices[3]);
		++performanceCounters.submittedQuads;

		rememberLogicalVertices(vertices, 4);
...
//////////////////////////////////////////////////////////////////////////
	...
	void GLES30Renderer::createContext()
	{
		...
		{
			const std::string error = "Requested OpenGL ES 3.0 but SDL created context " + std::to_string(contextMajor) + "." + std::to_string(contextMinor);
			LOG(LogError) << error;
			SDL_GL_DeleteContext(sdlContext);
			sdlContext = nullptr;
			throw std::runtime_error(error);
...
//////////////////////////////////////////////////////////////////////////
	...
	void GLES30Renderer::drawTriangleStrips(const Vertex* _vertices, const unsigned int _numVertices, const Blend::Factor _srcBlendFactor, const Blend::Factor _dstBlendFactor, bool verticesChanged)
	{
		...
		if (boundTexture != 0)
		{
			auto texture = _textures.find(boundTexture);
			batchableTexture = texture != _textures.cend() && texture->second != nullptr && !isAlphaTexture(texture->second->type);
		...
		if (boundTexture != 0)
		{
			...
			if (it != _textures.cend() && it->second != nullptr && isAlphaTexture(it->second->type))
...
//////////////////////////////////////////////////////////////////////////
	...
	void GLES30Renderer::swapBuffers()
	{
		...
		GLES30_CALL(glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT));
	} // swapBuffers
...
//////////////////////////////////////////////////////////////////////////
	...
	void GLES30Renderer::setStencil(const Vertex* _vertices, const unsigned int _numVertices)
	{
		...
		GLES30_CALL(glClear(GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT));

		GLES30_CALL(glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE));
		GLES30_CALL(glDepthMask(GL_FALSE));
		...
		GLES30_CALL(glDrawArrays(GL_TRIANGLE_FAN, range.firstVertex, range.vertexCount));
		setBlendState(false);

		GLES30_CALL(glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE));
		GLES30_CALL(glDepthMask(GL_TRUE));
	...
	void GLES30Renderer::postProcessShader(const std::string& path, const float _x, const float _y, const float _w, const float _h, const std::map<std::string, std::string>& parameters, unsigned int* data)
	{
		...
		if (nTextureID > 0)
		{
			...
			for (int i = 0; i < shaderBatch->size(); i++)
			{
				...
				setBlendState(false);
				GLES30_CALL(glDrawArrays(GL_TRIANGLE_STRIP, postProcessRange.firstVertex, postProcessRange.vertexCount));
			}
